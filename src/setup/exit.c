...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/19 18:57:29 by dabalm            #+#    #+#             */
/*   Updated: 2023/12/20 15:41:29 by dabalm           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_hitbox(t_game *game)
{
	...
	game->exit->hitbox.bottomright.x = game->exit->position.x + (game->size
			/ 4);
	game->exit->hitbox.bottomright.y = game->exit->position.y + (game->size
			/ 2);
}
...
void	setup_exit(t_game *game)
{
	...
	if (game->exit->position.x == 0 && game->exit->position.y == 0)
	{
		free_all(game, "no exit :(");
	}
	srand(rand() + game->map->height / game->map->width + game->size
		+ game->player->x);
	if (game->map->matrix[game->exit->position.y
			+ 1][game->exit->position.x] == '1')
		temp = 0;
	else
		temp = (rand() % (game->size - (game->size / 4)));
	game->exit->position.x *= game->size;
	game->exit->position.y *= game->size;
	game->exit->position.x += (rand() % (2 * (game->size / 3))) + (game->size
			/ 6);
