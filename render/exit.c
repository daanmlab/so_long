...
/*   Updated: 2023/12/09 18:19:17 by dabalm           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_exit(t_game *game)
{
    ...
    if (
        (game->exit->position.x / game->size) >= frame_x_pos
        ...
        put_img(game, game->exit_imgs[game->exit->frame], 
            (game->exit->position.x - ((frame_x_pos * game->size) + game->size / 2)),
            (game->exit->position.y - ((frame_y_pos * game->size) + game->size / 2))
        );
